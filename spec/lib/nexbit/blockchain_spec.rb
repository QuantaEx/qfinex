describe Nexbit::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Nexbit::Blockchain.new
      expect(blockchain1.features).to eq Nexbit::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Nexbit::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it 'custom feautures' do
      blockchain3 = Nexbit::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Nexbit::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Nexbit::Blockchain.new }
  ...
  context :latest_block_number do
    ...
    let(:blockchain) do
      Nexbit::Blockchain.new.tap {|b| b.configure(server: server)}
    end
  ...
  context :fetch_block! do
    ...
    let(:currency) do
      Currency.find_by(id: :nbc)
    end

    let(:server) { 'http://user:password@127.0.0.1:18332' }
    let(:endpoint) { 'http://127.0.0.1:18332' }
    let(:blockchain) do
      Nexbit::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
  ...
  context :build_transaction do
    ...
    let(:blockchain) do
      Nexbit::Blockchain.new.tap { |b| b.configure(currencies: [currency.to_blockchain_api_settings]) }
    end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Nexbit::Blockchain.new.tap do |b|
  ...
  context :load_balance_of_address! do
    ...
    context 'address with balance is defined' do
      it 'requests rpc listaddressgroupings and finds address balance' do
        ...
        result = blockchain.load_balance_of_address!(address, :nbc)
    ...
    context 'address is not defined' do
      it 'requests rpc listaddressgroupings and do not find address' do
        ...
        expect{ blockchain.load_balance_of_address!(address, :nbc)}.to raise_error(Peatio::Blockchain::UnavailableAddressBalanceError)
      end
    end
    ...
    context 'client error is raised' do
      ...
      it 'raise wrapped client error' do
        expect{ blockchain.load_balance_of_address!('anything', :nbc)}.to raise_error(Peatio::Blockchain::ClientError)
      end
    end
  end
end
